...
# You can also run it without building the PostgresDB project (if it is already built):
# ./run_browser_tests.sh -skipPostgresDBBuild
...
# Build server
if [[ $1 = "-skipPostgresDBBuild" ]]
then
  echo "Skipping PostgresDB build"
else
...
yarn workspace @daxon/react-components run build
