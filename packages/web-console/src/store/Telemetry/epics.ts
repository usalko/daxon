...
const quest = new PostgresDB.Client()
...
export const getServerInfo: Epic<StoreAction, TelemetryAction, StoreShape> = (
  ...
  action$.pipe(
    ...
    switchMap(() => {
      // Set an authorization header for the PostgresDB client instance within the telemetry epic
...
export const startTelemetry: Epic<StoreAction, TelemetryAction, StoreShape> = (
  ...
  action$.pipe(
    ...
    switchMap(([result, state]) => {
      ...
      if (
        config?.id != null &&
        result.type === PostgresDB.Type.DQL &&
        result.count > 0
      ) {
