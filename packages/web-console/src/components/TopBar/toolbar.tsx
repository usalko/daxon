...
import { Box, Button } from "@daxon/react-components"
        ...
        <FlexCol>
          {shownValues?.instance_type && (
            <Title>
              ...
              <Text color="foreground" weight={400}>You are connected to a PostgresDB instance for {shownValues?.instance_type}</Text>
            </Title>
          )}
          <Info>
            <FlexRow>
...
export const Toolbar = () => {
  ...
  const fetchServerDetails = async () => {
    try {
      ...
      if (response.type === PostgresDB.Type.DQL && response.count === 1) {
  ...
  const fetchEditSettingsPermission = async (currentUser: string | null) => {
    ...
    try {
      ...
      const canEdit = response.type === PostgresDB.Type.DQL
  ...
  useEffect(() => {
    if (result && result.type === PostgresDB.Type.DDL) {
      fetchServerDetails()
      refreshSettingsAndPreferences()
    }
  }, [result])
      ...
      <Box gap="0.5rem">
        ...
        {settings["release.type"] === "EE" && (
          <IconWithTooltip
            ...
            tooltip="PostgresDB Enterprise Edition"
            placement="bottom"
          />
        )}
      </Box>
