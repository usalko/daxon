...
const Schema = ({
  ...
  const [tables, setTables] = useState<PostgresDB.Table[]>()
  const [walTables, setWalTables] = useState<PostgresDB.WalTable[]>()
  const [materializedViews, setMaterializedViews] = useState<PostgresDB.MaterializedView[]>()
  ...
  const fetchTables = async () => {
    try {
      ...
      if (response && response.type === PostgresDB.Type.DQL) {
        errorRef.current = null
        setTables(response.data)
        dispatch(actions.query.setTables(response.data))
        // Fetch WAL info about the tables
        const walTablesResponse = await quest.query<PostgresDB.WalTable>(
          "wal_tables()",
        )
        if (walTablesResponse && walTablesResponse.type === PostgresDB.Type.DQL) {
  ...
  const fetchMaterializedViews = async () => {
    try {
      const matViewsResponse = await quest.query<PostgresDB.MaterializedView>(
        "materialized_views()",
      )
      if (matViewsResponse && matViewsResponse.type === PostgresDB.Type.DQL) {
  ...
  const fetchColumns = async () => {
    ...
    for (const query of queries) {
      try {
        const response = await quest.query<PostgresDB.InformationSchemaColumn>(
          query,
        )

        if (response?.type === PostgresDB.Type.DQL) {
    ...
    const ddls = await Promise.all(
      selectedTables.map(async (table) => {
        try {
          ...
          if (tableDDLResponse && tableDDLResponse.type === PostgresDB.Type.DQL) {
