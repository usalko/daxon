...
type VirtualTablesProps = {
  tables: PostgresDB.Table[]
  walTables?: PostgresDB.WalTable[]
  materializedViews?: PostgresDB.MaterializedView[]
...
export type FlattenedTreeItem = {
  ...
  name: string
  value?: string
  table?: PostgresDB.Table
  column?: PostgresDB.Column
  matViewData?: PostgresDB.MaterializedView
  walTableData?: PostgresDB.WalTable
  parent?: string
  isExpanded?: boolean
  isLoading?: boolean
...
const VirtualTables: FC<VirtualTablesProps> = ({
  ...
  const [regularTables, matViewTables] = useMemo(() => {
    const filtered = tables.filter((table: PostgresDB.Table) => {
    ...
    return filtered.reduce((acc, table) => {
      ...
      return acc
    }, [[], []] as PostgresDB.Table[][]).map(tables => 
  ...
  const handleCopyQuery = async (tableName: string, isMatView: boolean) => {
    try {
      ...
      if (response?.type === PostgresDB.Type.DQL && response.data?.[0]?.ddl) {
