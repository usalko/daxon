...
const MonacoEditor = () => {
  ...
  const onMount = (editor: editor.IStandaloneCodeEditor, monaco: Monaco) => {
    ...
    editor.setModel(
      monaco.editor.createModel(activeBuffer.value, PostgresDBLanguageName),
    )
    setEditorReady(true)
      ...
      if (request?.query) {
        ...
        void quest
          ...
          .then((result) => {
            ...
            if (
              result.type === PostgresDB.Type.DDL ||
              result.type === PostgresDB.Type.DML
            ) {
              dispatch(
                actions.query.addNotification({
            ...
            if (result.type === PostgresDB.Type.NOTICE) {
              dispatch(
                actions.query.addNotification({
                  content: (
            ...
            if (result.type === PostgresDB.Type.DQL) {
  ...
  return (
    <Content onClick={handleEditorClick}>
      <Editor
        ...
        defaultLanguage={PostgresDBLanguageName}
        onMount={onMount}
        saveViewState={false}
        onChange={(value) => {
