...
import { Box, Button, ForwardRef, Popover } from "@daxon/react-components"
  ...
  const fetchMetric = async (overwrite?: boolean) => {
    ...
    try {
      ...
      const responses = await Promise.all<
        | PostgresDB.QueryResult<ResultType[MetricType]>
        | PostgresDB.QueryResult<LastNotNull>
      >([
      ...
      if (responses[0] && responses[0].type === PostgresDB.Type.DQL) {
      ...
      if (responses[1] && responses[1].type === PostgresDB.Type.DQL) {
